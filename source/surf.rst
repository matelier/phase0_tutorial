...
まずはこの構造の最適化を行います。
例題の入力ファイルは ``Si001/relax`` の下にあります。
...
ディレクトリ ``exercise`` に、初期原子配置が異なる入力ファイルを用意しました。
それぞれ計算して、収束した原子配置を確認してみましょう。
最もエネルギーが低い配置はどれですか。
