...
並列計算は、nfinp.dataファイルやfile_names.dataファイルのあるディレクトリに移り、mpiexecコマンドを介して実行します。
たとえば以下のようなコマンドになります。
...
その変換のためのPythonスクリプトconv.pyはPHASE/0インストールディレクトリの下のbinディレクトリに含まれています。
...
.. table:: 本チュートリアルで用いたLinuxコマンド
   ...
   |                      | 例：                                         |
   |                      |                                              |
   |                      | cd abc (abcというディレクトリに移る)         |
   |                      |                                              |
   |                      | cd (ホームディレクトリに移る)                |
   |                      |                                              |
   +----------------------+----------------------------------------------+
   | ls                   | ディレクトリの下のファイルを表示する。       |
   |                      |                                              |
   |                      | 例：                                         |
   |                      |                                              |
   |                      | $ ls                                         |
